...
## Pricing Job (Phase 1-B-3, placeholder)

trigger 로 평가 Job 을 만들어 상태 전이(QUEUED→RUNNING→DONE)시키고 **placeholder PricingResult** 를 저장·조회한다.
**resolve(커브/자산 치환)와 엔진 계산은 아직 더미**다(인터페이스만 정식, Phase 3 에서 실제 구현으로 교체).

```bash
# 평가 실행(VALUATOR+). 본문은 trigger(model?/seed?/options?/params_override?) — rawForm 전체 아님
curl -X POST localhost:8080/instruments/1/price -H "Authorization: Bearer $TOKEN" \
  -H 'Content-Type: application/json' -d '{}'
#   → 201 {"job_id":1,"status":"DONE","cached":false}

curl localhost:8080/jobs/1 -H "Authorization: Bearer $TOKEN"          # 상태·메타
curl localhost:8080/jobs/1/result -H "Authorization: Bearer $TOKEN"   # placeholder PricingResult
#   components 전부 0, total_fair_value 0(Σ=total), warnings:[{code:PLACEHOLDER}], reproducibility.input_hash(진짜)
```

**파이프라인 ("자리는 잡되 속은 더미")**:
`trigger → terms_json 로드 → ContextResolver.resolve → InputHash(진짜) → 캐시조회 → PricingEngineClient.price → result 저장 → DONE`.
`JobService` 는 **`ContextResolver`/`PricingEngineClient` 인터페이스에만 의존**한다 — Phase 3 에서 `RealContextResolver`(커브 *_ref→포인트, asset→spot/vol)·`FastApiPricingEngineClient`(FastAPI /price)로 교체해도 호출부·흐름·테스트가 바뀌지 않는다.

- **input_hash 는 진짜**: 기존 `InputHash` 로 산출·저장. 동일 입력 재요청은 `cached=true` 로 같은 결과 재사용.
- **상태**: QUEUED→RUNNING→DONE 동기 처리(실패 시 FAILED). 평가 후 instrument.status=PRICED.
